...
def dead_reckoning(csv_path):
    ...
    # Initialize orientation quaternion from first sample's magnetometer reading
    if 'magnetic_compass' in df.columns and pd.notna(df['magnetic_compass'].iloc[0]):
        initial_yaw = np.radians(df['magnetic_compass'].iloc[0])
        q = quat_from_euler(0.0, 0.0, initial_yaw)  # Level start, correct heading
        print(f"Initial heading: {df['magnetic_compass'].iloc[0]:.1f}° from magnetometer")
    else:
    ...
    for i in range(len(df)):
        ...
        # CORRECTED MAPPING (found via systematic testing)
        accel_x = accel_z_msfs    # MSFS Z (forward) → Standard X (forward)
        accel_y = -accel_x_msfs   # MSFS X (right) → Standard Y (right), INVERTED
        accel_z = -accel_y_msfs   # MSFS Y (up) → Standard Z (down), inverted
        
        # Remap gyro rates to match (same frame convention)
        gyro_x_msfs = df['gyro_x'].iloc[i]  # rad/s
        gyro_y_msfs = df['gyro_y'].iloc[i]
        gyro_z_msfs = df['gyro_z'].iloc[i]
        
        gyro_x = gyro_z_msfs    # MSFS Z (yaw around forward) → Standard X (roll)
        gyro_y = -gyro_x_msfs   # MSFS X (roll around right) → Standard Y (pitch), INVERTED
        gyro_z = -gyro_y_msfs   # MSFS Y (pitch around up) → Standard Z (yaw), inverted
        ...
        if dt > 0:
            ...
            if 'magnetic_compass' in df.columns and pd.notna(df['magnetic_compass'].iloc[i]):
                heading_deg = df['magnetic_compass'].iloc[i]
                target_yaw = np.radians(heading_deg)
            ...
            # Remove gravity (pointing down in NED)
            accel_ned[2] -= 9.81
            ...
            if 'airspeed_true' in df.columns and pd.notna(df['airspeed_true'].iloc[i]):
                ...
                # Use magnetic compass DIRECTLY for heading (most accurate)
