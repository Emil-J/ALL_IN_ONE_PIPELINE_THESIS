...
class ErrorStateEKF:
    ...
    def __init__(self, lat0, lon0, alt0, heading0):
        """Initialize EKF
        ...
        self.P[3:6, 3:6] = np.eye(3) * 0.001  # Very small initial bias uncertainty (conservative)
        ...
        # Magnetic field at Vejle, Denmark (approximate)
        # Declination ~3° East, Inclination ~70° down
        mag_dec = np.radians(3.0)
        mag_inc = np.radians(70.0)
        self.m_n = np.array([
            np.cos(mag_inc) * np.cos(mag_dec),
            np.cos(mag_inc) * np.sin(mag_dec),
            np.sin(mag_inc)
        ])
        ...
        self.Q_gyro_bias = np.eye(3) * (0.01)**2  # Gyro bias drift (rad/s)² - moderate for online estimation
        ...
        self.R_mag = np.eye(3) * (0.1)**2    # Magnetometer noise - TRUST MORE (reduced from 0.2 for stronger heading correction)
        ...
        self.heading_correction_gain = 0.15  # Proportional gain for magnetometer heading correction
    ...
    def predict(self, omega_meas, accel_body, dt):
        ...
        # pos_d updated from barometer in measurement update
    
    
    def update_accel_mag(self, accel_meas, mag_heading_deg):
        """Measurement update from accelerometer and magnetometer
        
        Args:
            accel_meas: Accelerometer measurement (m/s²) in body frame
            mag_heading_deg: Magnetometer heading (degrees, 0=North, 90=East)
        """
        # ═══ ACCELEROMETER MEASUREMENT MODEL (Equation 4.51) ═══
        # Measurement: y_a = R_bn * g_n + noise
        # Linearized: y_a ≈ R̃_bn * g_n + R̃_bn * [g_n]× * δθ + noise
        ...
        # Measurement Jacobian: H_a = R̃_bn * [g_n]× for orientation (no bias sensitivity, no wind sensitivity)
        H_accel = R_bn @ skew(self.g_n)
        H_accel_full = np.hstack([H_accel, np.zeros((3, 3)), np.zeros((3, 2))])  # 3x8: [∂h/∂θ, ∂h/∂b, ∂h/∂w] = [H_accel, 0, 0]
        
        # ═══ MAGNETOMETER MEASUREMENT MODEL ═══
        # We only have heading, so project to horizontal plane
        # Expected heading from current orientation
        m_body_expected = R_bn @ self.m_n
        heading_expected = np.arctan2(m_body_expected[1], m_body_expected[0])
        ...
        # Measurement Jacobian for heading (simplified - only affects yaw, no bias sensitivity, no wind sensitivity)
        # H_mag for yaw-only: [0, 0, 1, 0, 0, 0, 0, 0] (correction around Z-axis only)
        H_mag_full = np.array([[0, 0, 1, 0, 0, 0, 0, 0]])  # 1x8 matrix
        
        # ═══ COMBINED MEASUREMENT UPDATE ═══
        # Stack measurements and Jacobians
        y_innovation = np.hstack([innovation_accel, innovation_heading])
        H = np.vstack([H_accel_full, H_mag_full])  # 4x8 matrix (4 measurements, 8 states)
        
        # Measurement noise covariance
        R = np.block([
            [self.R_accel, np.zeros((3, 1))],
            [np.zeros((1, 3)), np.array([[self.R_mag[0, 0]]])]  
        ])  # 4x4 matrix
        
        # ═══ KALMAN UPDATE (Equation 4.55a, 4.55b extended to 8D) ═══
        ...
        # State update: [δθ, δb, δw]^T = K * innovation
        delta_state = K @ y_innovation  # 8D vector: [δθ_x, δθ_y, δθ_z, δb_x, δb_y, δb_z, δw_n, δw_e]
        
        # Extract orientation, bias, and wind corrections
        delta_theta = delta_state[0:3]  # Orientation error correction
        delta_bias = delta_state[3:6]   # Bias error correction
        delta_wind = delta_state[6:8]   # Wind error correction
        
        # Covariance update: P̃ = P - K * S * K^T
        self.P = self.P - K @ S @ K.T
        
        # ═══ APPLY STATE CORRECTIONS ═══
        # Update quaternion: q̃(t|t) = exp_q(δ̂θ/2) * q̃(t|t-1)
        dq = expq(delta_theta / 2)
        self.q_tilde = quat_multiply(dq, self.q_tilde)
        self.q_tilde = quat_normalize(self.q_tilde)
        
        # Update bias estimate: b̂ = b̂ + δb
        self.gyro_bias += delta_bias
        
        # Update wind estimate: ŵ = ŵ + δw
        self.wind_n += delta_wind[0]
        self.wind_e += delta_wind[1]
        # This complements the EKF measurement update with a simpler, more direct correction
        if mag_heading_deg is not None:
            # Get current yaw from quaternion
            _, _, quat_yaw = quat_to_euler(self.q_tilde)
            
            # Measured heading from magnetometer
            mag_yaw = np.radians(mag_heading_deg)
            
            # Heading error (wrapped to [-π, π])
            heading_error = mag_yaw - quat_yaw
            heading_error = np.arctan2(np.sin(heading_error), np.cos(heading_error))
            
            # Apply proportional correction (complementary filter style)
            correction_angle = self.heading_correction_gain * heading_error
            
            # Create correction quaternion (rotation around Z-axis only)
            correction_quat = np.array([
                np.cos(correction_angle / 2),
                0,
                0,
                np.sin(correction_angle / 2)
            ])
            
            # Apply correction
            self.q_tilde = quat_multiply(self.q_tilde, correction_quat)
            self.q_tilde = quat_normalize(self.q_tilde)
    ...
    def update_airspeed(self, airspeed_meas, mag_heading_deg=None):
        ...
        if self.in_maneuver:
            # During maneuvers: low trust in airspeed (centripetal accel present)
            alpha = 0.2  # 20% airspeed, 80% integration
        elif self.wind_converged:
            # Straight flight + converged wind: high trust in airspeed
            alpha = 0.8  # 80% airspeed, 20% integration
        else:
            # Straight flight + converging wind: balanced blend
...
def run_ekf_ins(input_file, output_file=None):
    ...
    if 'magnetic_compass' not in df.columns:
        raise ValueError("Magnetometer (magnetic_compass) required for INS heading reference")
    ...
    # Use magnetic_compass for heading
    heading0 = df['magnetic_compass'].iloc[0]
    ...
    # Create EKF
    ekf = ErrorStateEKF(lat0, lon0, alt0, heading0)
    ...
    for i in range(len(df)):
        ...
        # MSFS: X=right, Y=up, Z=forward → Standard: X=forward, Y=right, Z=down
        ...
        accel_body = np.array([
            ...
            -accel_x_msfs,   # MSFS X (right) → Standard Y, inverted
        ...
        omega_meas = np.array([
            gyro_z_msfs,     # MSFS Z → Standard X
            -gyro_x_msfs,    # MSFS X → Standard Y, inverted
            -gyro_y_msfs     # MSFS Y → Standard Z, inverted
        ])
        ...
        # Use magnetic_compass for heading
        mag_heading = df['magnetic_compass'].iloc[i]
        ...
        if 'airspeed_true' in df.columns and pd.notna(df['airspeed_true'].iloc[i]):
            ...
            airspeed = airspeed_knots * 0.514444  # Convert knots → m/s 
