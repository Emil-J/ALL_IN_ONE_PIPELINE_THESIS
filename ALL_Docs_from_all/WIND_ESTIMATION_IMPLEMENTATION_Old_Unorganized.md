...
3. [Initial Problem State](#initial-problem-state)
4. [Root Cause Analysis](#root-cause-analysis)
5. [Solution Design: 8D EKF Architecture](#solution-design-8d-ekf-architecture)
6. [Implementation Phase 1: State Vector Extension](#implementation-phase-1-state-vector-extension)
7. [Critical Bug Discovery: Wind Observability](#critical-bug-discovery-wind-observability)
8. [Implementation Phase 2: Proper Kalman Measurement](#implementation-phase-2-proper-kalman-measurement)
9. [Critical Bug Discovery: Velocity Initialization](#critical-bug-discovery-velocity-initialization)
10. [Implementation Phase 3: Smart Initialization](#implementation-phase-3-smart-initialization)
11. [Final Architecture and Data Flow](#final-architecture-and-data-flow)
12. [Theoretical Foundation](#theoretical-foundation)
13. [Performance Expectations](#performance-expectations)
14. [Code Changes Summary](#code-changes-summary)
...
[57] Hamm mond, J. K., & Papadopou, P. R. (1978). "Observability and Controllability for Systems with External Functions." *International Journal of Control*, 28(6), 889-896.
...
Continuous-time dynamics:
  ẋ = f(x, u, t) + w(t)    [Process model]
  z = h(x, t) + v(t)       [Measurement model]

where:
  x = state vector (8D in our case)
  u = control input (IMU measurements)
  w = process noise (Q covariance)
  z = measurement vector (airspeed, accel, mag, baro)
...
1. PREDICTION (Time Update):
   x̂⁻ₖ = f(x̂ₖ₋₁, uₖ₋₁)              [Propagate state]
   Pₖ⁻ = FₖPₖ₋₁Fₖᵀ + Qₖ               [Propagate covariance]
   
   where:
   Fₖ = ∂f/∂x |ₓ₌ₓ̂ₖ₋₁                [State transition Jacobian]

2. MEASUREMENT UPDATE (Correction):
   yₖ = zₖ - h(x̂ₖ⁻)                  [Innovation]
   Sₖ = HₖPₖ⁻Hₖᵀ + Rₖ                 [Innovation covariance]
   Kₖ = Pₖ⁻Hₖᵀ Sₖ⁻¹                  [Kalman gain]
   x̂ₖ = x̂ₖ⁻ + Kₖyₖ                   [State correction]
   Pₖ = (I - KₖHₖ)Pₖ⁻                [Covariance correction]
   
   where:
   Hₖ = ∂h/∂x |ₓ₌ₓ̂ₖ⁻                [Measurement Jacobian]
```
...
Instead of estimating full state x directly, estimate error δx:

```
True state: x_true = x_nominal + δx

Benefits:
1. δx remains small → linearization valid
2. Nominal state obeys nonlinear physics exactly
3. Covariance for small errors is well-behaved
4. Quaternion singularities avoided
```

**Our Implementation:**

```
Nominal state (integrated directly):
  • Position: p = p + v*dt
  • Velocity: v = v + a*dt
  • Orientation: q = integrate_quaternion(q, ω, dt)
  • Bias: b (constant)
  • Wind: w (constant)

Error state (EKF estimates):
  • δθ: small-angle orientation error (3D)
  • δb: bias error (3D)
  • w: wind (2D) ← Actually nominal, not error!

Mixture: Orientation and bias are error-state, wind is nominal
This works because wind can be large (57 m/s not small)
```

### Observability Analysis

**Definition:** State x is observable if it can be determined from measurements z(t) over time.

**Rank Condition:**

For linear system ẋ = Ax, z = Cx:
```
Observability matrix O = [C; CA; CA²; ...; CA^(n-1)]
Observable ⟺ rank(O) = n
```

**Wind Observability in 8D EKF:**
...
Jacobian:
H = [0 0 0 0 0 0 1 0]  (∂h_n/∂w_n = 1)
    [0 0 0 0 0 0 0 1]  (∂h_e/∂w_e = 1)

Observability:
Rank(H) = 2 for wind dimensions → wind is observable ✓
```

**Why Two Velocity Sources Are Required:**

```
If only airspeed:
  z = airspeed
  h(v, w) = |v + w|  (magnitude only)
  
  Infinite solutions: many (v, w) pairs give same |v+w|
  Example: (v=100, w=0) and (v=50, w=50) both give |v+w|=100
  Not observable!

If airspeed + velocity:
  z₁ = airspeed vector (air-relative)
  z₂ = velocity vector (ground-relative)
  
  Single solution: w = z₁ - z₂
  Observable! ✓
```
...
Physical meaning:
  • K: How much to trust measurement vs prediction
  • P: Prediction uncertainty (high = trust measurement)
  • R: Measurement noise (high = trust prediction)
  • H: Sensitivity of measurement to states
```

**For Wind Estimation:**

```
High wind uncertainty (P[6:8,6:8] large):
  → Large Kalman gain for wind
  → Trust airspeed measurement heavily
  → Wind estimate changes quickly

Low wind uncertainty (P[6:8,6:8] small):
  → Small Kalman gain for wind
  → Trust current wind estimate
  → Wind changes slowly (filtered)

After convergence:
  → P[6:8,6:8] stabilizes
  → K reaches steady-state
  → Wind tracks slowly-varying actual wind
```
...
4. **FAA (2016).** Pilot's Handbook of Aeronautical Knowledge. Chapter 8: Flight Instruments.
   - Airspeed indicator operation (pitot-static system)
   - True airspeed vs ground speed
   - Wind effect on navigation

5. **Anderson, J. D. (2016).** Introduction to Flight (8th ed.). McGraw-Hill.
   - Aircraft performance in wind
   - Ground speed = airspeed ± wind component
   - Coordinate systems (body, wind, earth)

### Software Implementation

6. **NumPy Documentation (2024).** Linear Algebra Operations.
   - Matrix operations for EKF
   - Eigenvalue decomposition
   - Numerical stability considerations

7. **Pandas Documentation (2024).** DataFrame Operations.
   - Time-series data handling
   - CSV I/O for sensor logs
   - Data preprocessing

### External Resources

8. **Microsoft Flight Simulator SDK (2020).** SimConnect API Reference.
   - Sensor data acquisition (SimVars)
   - Coordinate system conventions
   - Aircraft state variables

9. **GitHub Copilot Session Logs (2026-03-11).** Wind Estimation Implementation.
